...
export const FtlWorkflowSection: React.FC = () => {
  ...
  return (
    <section id="ftl-workflow" className="py-10 sm:py-16 md:py-24 bg-slate-900 text-white border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-16">
          ...
          <p className="text-slate-400 mt-2 text-sm sm:text-base lg:text-lg font-urdu">
            {t.subtitle}
          </p>
        </div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-8 sm:mb-12 ${language === 'ur' ? 'text-right' : 'text-left'}`}>
          {workflowSteps.map((s, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="bg-slate-950 border border-slate-800 hover:border-amber-500/50 rounded-xl p-5 relative transition-all duration-300 shadow-md flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-3.5">
                    ...
                    <span className="bg-slate-900 text-slate-300 border border-slate-800 text-[10px] font-semibold px-2 py-0.5 rounded font-urdu">
                      {language === 'ur' ? s.tagUrdu : s.tagEnglish}
                    </span>
                  </div>

                  <div className={`w-10 h-10 rounded-lg bg-slate-900 text-amber-400 flex items-center justify-center mb-3.5 border border-slate-800 ${language === 'ur' ? 'ml-auto' : 'mr-auto'}`}>
                    <Icon className="w-5 h-5" />
                  </div>

                  <h3 className="text-base font-bold text-white font-urdu mb-1.5">
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 sm:p-8 shadow-xl">
          
          <div className={`flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 sm:gap-6 border-b border-slate-800 pb-5 sm:pb-6 mb-6 sm:mb-8 ${language === 'ur' ? 'text-right' : 'text-left'}`}>
            <div>
              <div className="inline-flex items-center gap-2 bg-slate-900 text-amber-400 border border-amber-500/30 px-3 py-1 rounded-md text-xs font-semibold font-urdu mb-2">
                <ShieldCheck className="w-3.5 h-3.5 flex-shrink-0" />
                <span>{language === 'ur' ? 'کاروباری اداروں اور فیکٹریوں کے لیے بہترین انتخاب' : 'Best Choice for Commercial Shippers'}</span>
              </div>
              <h3 className="text-xl sm:text-2xl font-extrabold text-white font-urdu">
                {language === 'ur' ? 'فل ٹرک لوڈ (FTL) کے اہم فوائد' : 'Core Dedicated Full Truckload (FTL) Advantages'}
              </h3>
            </div>

            <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 text-xs text-slate-300 max-w-md font-urdu">
              🔒 <strong className="text-amber-400">{language === 'ur' ? 'واضح پالیسی:' : 'Clear Policy:'}</strong> {language === 'ur' ? 'وڑائچ گڈز ٹرانسپورٹ کمپنی لوز کارگو / پارسل (LTL) بک نہیں کرتی۔ ہماری تمام گاڑیاں سنگل پارٹی کے مکمل لوڈ کے لیے وقف ہوتی ہیں۔' : 'Warraich Goods strictly does NOT accept loose parcels or mixed LTL cargo. All fleet trucks are 100% reserved for single-shipper full loads.'}
            </div>
          </div>
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${language === 'ur' ? 'text-right' : 'text-left'}`}>
            {ftlPillars.map((p, idx) => {
              const Icon = p.icon;
              return (
                <div key={idx} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-start gap-3.5">
                  <div className="w-9 h-9 rounded-lg bg-slate-950 border border-slate-800 text-amber-400 flex items-center justify-center flex-shrink-0 mt-0.5">
          ...
          {/* Direct CTA */}
          <div className={`mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 text-xs font-urdu ${language === 'ur' ? 'text-right' : 'text-left'}`}>
            ...
            <a
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-5 py-3 rounded-xl transition cursor-pointer text-xs sm:text-sm font-urdu shadow-lg min-h-[44px]"
